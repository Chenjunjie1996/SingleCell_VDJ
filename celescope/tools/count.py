...
class Count(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        self.cell_matrix_dir = (
            f"{self.outdir}/{self.sample}_{FILTERED_MATRIX_DIR_SUFFIX[0]}"
        )
    ...
    def run(self):
        ...
        self.add_count_metrics(
            total_reads, total_cell_gene, cell_bc, df_bc_cell, downsample_dict
        )
        # plot
        self.add_plot_data()

    def add_count_metrics(
        self, total_reads, total_cell_gene, cell_bc, df_sum_cell, downsample_dict
    ):
        n_cells = len(cell_bc)
        self.add_metric(
        ...
        try:
            ...
            self.add_count_metrics.logger.warning(
                "Will not output `Mean Reads per Cell`"
            )
        else:
    ...
    def add_plot_data(self):
        ...
        line_saturation = Line_plot(
            ...
            x_title=X_TITLE,
            y_title=SATURATION_Y_TITLE,
            y_range=[0, 100],
            section=False,
        ).get_plotly_div()
        self.add_data(line_saturation=line_saturation)
        line_median = Line_plot(
            df_line=df_line,
            title="Median Genes per Cell",
            x_title=X_TITLE,
            y_title=MEDIAN_GENE_Y_TITLE,
        ).get_plotly_div()
    ...
    def sub_sample(fraction, df_cell, new_df, cell_read_index):
        ...
        geneNum_median = float(
            df_cell_subsample.groupby("Barcode").agg({"geneID": "nunique"}).median()
        )
    ...
    def downsample(self, df_cell):
        ...
        for fraction in np.arange(0.1, 1.1, 0.1):
            saturation, geneNum_median = Count.sub_sample(
                fraction, df_cell, new_df, cell_read_index
            )
