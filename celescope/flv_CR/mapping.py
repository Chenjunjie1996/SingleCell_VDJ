...
class Mapping(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        if self.match_dir != "None":
            self.contig_file = glob.glob(
                f"{args.match_out}/matched_contig_annotations.csv"
            )[0]
...
def get_opts_mapping(parser, sub_program):
    if sub_program:
        ...
        parser.add_argument(
            "--match_out", help="assemble result from match directory", required=True
        )
    return parser
