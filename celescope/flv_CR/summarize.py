...
class Summarize(Step):
    ...
    def convert_barcode_to_SGR(self):
        ...
        # all contig annotation
        self.df_all_annotation["barcode"] = self.df_all_annotation["barcode"].apply(
            lambda x: self.tenX_sgr[x.split("-")[0]]
        )
        self.df_all_annotation["contig_id"] = self.df_all_annotation["contig_id"].apply(
            lambda x: self.tenX_sgr[x.split("-")[0]]
            + "_"
            + x.split("_")[1]
            + "_"
        ...
        self.df_annotation["contig_id"] = self.df_annotation["contig_id"].apply(
            lambda x: self.tenX_sgr[x.split("-")[0]]
            + "_"
            + x.split("_")[1]
            + "_"
        ...
        if int(self.version.split(".")[0]) < 4:
            self.df_annotation["productive"].replace(
                {"True": True, "None": False}, inplace=True
            )

        self.df_annotation.to_csv(
            self.filtered_contig_annotations, sep=",", index=False
        )
    ...
    def gen_productive_contig(annotation_file, fasta_file, outdir, prefix=""):
        ...
        productive_contig.to_csv(
            f"{outdir}/{prefix}productive_contig_annotations.csv", sep=",", index=False
        )
    ...
    def run(self):
        ...
        Summarize.gen_productive_contig(
            self.df_annotation, self.filter_contig_fasta, self.outdir
        )
...
def get_opts_summarize(parser, sub_program):
    ...
    parser.add_argument(
        "--not_refine", help="Do not perform the refine step. ", action="store_true"
    )
    if sub_program:
        s_common(parser)
        ...
        parser.add_argument(
            "--assemble_out",
            help="directory of cellranger assemble result",
            required=True,
        )
    return parser
