...
class Filter_noise:
    ...
    def __call__(self):
        df_sorted = self.df.sort_values(
            ["barcode", "chain", "umis"], ascending=[False, False, False]
        )
        ...
        for _group_name, group_data in grouped_barcode:
            if len(group_data["chain"].unique()) == 2:
                if self.seqtype == "BCR" and "IGH" not in group_data["chain"].unique():
                    continue
                contig_id_set.update(
                    set(group_data["contig_id"])
                )  # 记录保留的contig_id

        # 根据contig_id确定需要保留的行
        final_filtered_data = filtered_data[
            filtered_data["contig_id"].isin(contig_id_set)
        ]
...
class Refine_assemble:
    ...
    def __init__(self, args):
        ...
        # in
        self.raw_all_contig_anno = pd.read_csv(
            f"{self.outdir}/{self.sample}/outs/all_contig_annotations.csv"
        )
        self.raw_filter_contig_anno = pd.read_csv(
            f"{self.outdir}/{self.sample}/outs/filtered_contig_annotations.csv"
        )
        ...
        self.out_fq1_file = (
            f"{self.refine_fqs_dir}/{self.refine_sample}_S1_L001_R1_001.fastq.gz"
        )
        self.out_fq2_file = (
            f"{self.refine_fqs_dir}/{self.refine_sample}_S1_L001_R2_001.fastq.gz"
        )
    ...
    def refine_vdj(self):
        ...
        """
        df_productive = self.raw_all_contig_anno[
            self.raw_all_contig_anno["productive"] == True
        ]
        ...
        df_productive = self.raw_filter_contig_anno[
            self.raw_filter_contig_anno["productive"] == True
        ]
    ...
    def gen_refine_fqs(self, raw_assembled_cells, refined_contigs):
        ...
        with pysam.FastxFile(
            self.raw_convert_fq1, persist=False
        ) as fq1, pysam.FastxFile(self.raw_convert_fq2, persist=False) as fq2:
