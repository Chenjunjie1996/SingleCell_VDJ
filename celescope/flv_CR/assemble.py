...
class cellranger_metrics(Step):
    def __init__(self, args, display_title=None):
        ...
        cellranger_metrics_csv = (
            f"{self.outdir}/{self.out_name}/outs/metrics_summary.csv"
        )
...
class Cells(cellranger_metrics):
    def __init__(self, args, display_title=None):
        ...
        self.filter_contig_file = pd.read_csv(
            f"{self.outdir}/{self.out_name}/outs/filtered_contig_annotations.csv"
        )
    ...
    def add_metrics(self):
        ...
        for chain in self.chains:
            median_value = self.filter_contig_file[
                self.filter_contig_file["chain"] == chain
            ]["umis"].median()
    ...
    def Barcode_rank_plot(self):
        ...
        df_umi["barcode"] = df_umi["barcode"].apply(
            lambda x: self.tenX_sgr[x.split("-")[0]]
        )

        df_umi.to_csv(self.count_file, sep="\t", index=False)
        self.add_data(
            chart=get_plot_elements.plot_barcode_rank(self.count_file, log_uniform=True)
        )
...
def get_opts_assemble(parser, sub_program):
    ...
    parser.add_argument(
        "--other_param", help="Other cellranger parameters.", default=""
    )
