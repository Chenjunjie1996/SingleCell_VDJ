...
class Assemble(Step):
    ...
    def split_candidate_reads(self):
        ...
        fq_list = [
            open(f"{self.temp_outdir}/temp_{i}.fq", "w") for i in range(self._n_chunk)
        ]
        bc_list = [
            open(f"{self.temp_outdir}/temp_{i}_bc.fa", "w")
            for i in range(self._n_chunk)
        ]
        umi_list = [
            open(f"{self.temp_outdir}/temp_{i}_umi.fa", "w")
            for i in range(self._n_chunk)
        ]
    ...
    def run_assemble(self):
        ...
        with Pool(self._n_chunk) as pool:
            pool.starmap(
                ...
                zip(
                    self.temp_ref_list,
                    self.temp_outdir_list,
                    self.temp_name_list,
                    self.single_thread_list,
                ),
            )
    ...
    def merge_file(self):
        ...
        for file_suffix in file_suffixes:
            file_path = [
                f"{self.temp_outdir}/temp_{thread}_{file_suffix}"
                for thread in range(self._n_chunk)
            ]
...
def get_opts_assemble(parser, sub_program):
    if sub_program:
        ...
        parser.add_argument(
            "--candidate_fq",
            help="Candidate fastq file from mapping step",
            required=True,
        )
