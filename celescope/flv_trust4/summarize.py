...
def target_cell_calling(
    ...
    umi_threshold = Auto(
        list(df_UMI_sum[umi_col]),
        expected_cell_num=expected_target_cell_num,
        coef=coef,
    ...
    if target_barcodes:
        df_temp[umi_col] = df_temp.apply(
            lambda row: row[umi_col] * weight
            if row["barcode"] in target_barcodes
            else row[umi_col],
            axis=1,
        )
...
class Summarize(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        if self.diffuseFrac:
            self.barcode_report = (
                f"{args.assemble_out}/{self.sample}_{BARCODE_REPORT_FILTER_SUFFIX}"
            )
        else:
            self.barcode_report = (
                f"{args.assemble_out}/{self.sample}_{BARCODE_REPORT_SUFFIX}"
            )
        ...
        if not args.target_cell_barcode:
            ...
            self.target_barcodes, self.expected_target_cell_num = utils.read_one_col(
                args.target_cell_barcode
            )
    ...
    def _auto_assign(self):
        ...
        target_clusters = df_marker_top5[
            df_marker_top5["celltype"] == cell_type
        ].cluster.tolist()
        target_clusters = [int(x.split(" ")[-1]) for x in target_clusters]
        target_clusters = set(
            [x for x in target_clusters if target_clusters.count(x) > 1]
        )
    ...
    def parse_contig_file(self):
        ...
        """
        self.convert_barcode_report(
            self.barcode_report, outdir=f"{self.outdir}/{self.sample}"
        )
    ...
    def cell_calling(self, df):
        ...
        if self.seqtype == "BCR":
            ...
            df_chain_light = df_for_clono[
                (df_for_clono["chain"] == "IGK") | (df_for_clono["chain"] == "IGL")
            ]
        else:
    ...
    def parse_clonotypes(self, df, df_for_clono, cell_barcodes, filtered_contig):
        ...
        df_for_clono_pro["chain_cdr3nt"] = df_for_clono_pro.loc[
            :, ["chain", "cdr3_nt"]
        ].apply(":".join, axis=1)
        ...
        df_clonotypes = pd.read_csv(
            f"{self.outdir}/clonotypes.csv", sep="\t", index_col=None
        )
        contig_with_clonotype = copy.deepcopy(df_clonotypes)
        df_dict = (
            df_clonotypes[["cdr3s_nt", "cdr3s_aa"]]
            .set_index("cdr3s_nt")
            .to_dict(orient="dict")["cdr3s_aa"]
        )
        df_clonotypes = df_clonotypes.groupby("cdr3s_nt", as_index=False).agg(
            {"barcode": "count"}
        )
        df_clonotypes.rename(columns={"barcode": "frequency"}, inplace=True)
        sum_f = df_clonotypes["frequency"].sum()

        df_clonotypes["proportion"] = df_clonotypes["frequency"].apply(
            lambda x: x / sum_f
        )
        df_clonotypes.sort_values(by="frequency", ascending=False, inplace=True)
        df_clonotypes["clonotype_id"] = [
            f"clonotype{i}" for i in range(1, df_clonotypes.shape[0] + 1)
        ]
        df_clonotypes["cdr3s_aa"] = df_clonotypes["cdr3s_nt"].apply(
            lambda x: df_dict[x]
        )
        df_clonotypes = df_clonotypes.reindex(
        ...
        df_merge = pd.merge(
            used_for_merge, contig_with_clonotype, on="cdr3s_nt", how="outer"
        )
        ...
        df_all_contig.to_csv(
            f"{self.outdir}/{self.sample}_all_contig.csv", sep=",", index=False
        )
        df_filter_contig.to_csv(
            f"{self.outdir}/{self.sample}_filtered_contig.csv", sep=",", index=False
        )
    ...
    def gen_summary(self, df_for_clono):
        ...
        df_umi["mark"] = df_umi["barcode"].apply(
            lambda x: "CB" if x in cell_barcodes else "UB"
        )
        df_umi.to_csv(f"{self.outdir}/count.txt", sep="\t", index=False)
        self.add_data(
            chart=get_plot_elements.plot_barcode_rank(f"{self.outdir}/count.txt")
        )
        ...
        for c in self.chains:
            ...
            self.add_metric(name=f"Median {c} UMIs per Cell", value=median_umi)
...
def get_opts_summarize(parser, sub_program):
    ...
    if sub_program:
        ...
        parser.add_argument(
            "--assemble_out", help="Result of  assemble dirctory.", required=True
        )
        parser.add_argument(
            "--match_dir", help="Match scRNA-seq directory.", required=True
        )
