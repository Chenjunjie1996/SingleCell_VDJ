...
class Mapping(Step):
    ...
    def extract_chain_reads(self):
        ...
        with Pool(n_map) as pool:
            pool.starmap(
                Mapping.extract_candidate_reads,
                zip(
                    map_ref,
                    map_index_prefix,
                    map_outdirs,
...
def get_opts_mapping(parser, sub_program):
    if sub_program:
        ...
        parser.add_argument(
            "--match_fq2", help="R2 reads matched with scRNA-seq.", required=True
        )
        parser.add_argument(
            "--match_fq1", help="R1 reads matched with scRNA-seq.", required=True
        )
